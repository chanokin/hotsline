...
def timesurface(
    ...
    timestamp_memory = torch.zeros((sensor_size[2], sensor_size[1] + radius_y * 2, sensor_size[0] + radius_x * 2)).to(
        device
    )
    ...
    if ts_batch_size:
        ...
        all_surfaces = torch.zeros((ts_batch_size, sensor_size[2], surface_dimensions[1], surface_dimensions[0])).to(
            device
        )
        ...
        timestamp_memory -= tau * 5 + 1
        all_surfaces = torch.zeros((len(events), sensor_size[2], surface_dimensions[1], surface_dimensions[0])).to(
            device
        )
        events_list = events
    ...
    for index, event in enumerate(events_list):
        ...
        if radius_x > 0 and radius_y > 0:
            timestamp_context = (
                timestamp_memory[:, y : y + surface_dimensions[1], x : x + surface_dimensions[0]] - event[t_index]
            )
        else:
        ...
        if torch.nonzero(timesurface).shape[0] > filtering_threshold:
            if ts_batch_size:
                if indices.shape[0] >= ts_batch_size:
                    ...
                    if not drop_proba:
                        all_surfaces = (
                            torch.cat([all_surfaces, timesurface[None, :]], 0)
                            if all_surfaces.shape[0] > 0
                            else timesurface[None, :]
                        )
                        indices = (
                            torch.hstack([indices, torch.Tensor([index])])
                            if indices.shape[0] > 0
                            else torch.Tensor([index])
                        )
                    else:
